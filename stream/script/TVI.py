...
def update_tvi_m3u8():
    try:
        ...
        # 4️⃣ Adicionar BASE_URL nos links relativos
        def add_base_url(match):
            url = match.group(0)
            if url.startswith("http"):
                return url  # já é absoluto, não tocar
            return f"{BASE_URL}{url}"

        m3u8_content = re.sub(
            r'(?m)^(?!#)(\S+)$',  # linhas que não começam por # (são URLs/paths)
            add_base_url,
            m3u8_content
        )

        print(f"📄 Conteúdo m3u8 processado ({len(m3u8_content)} caracteres).")

        # 5️⃣ Guardar no ficheiro TVI.m3u8
